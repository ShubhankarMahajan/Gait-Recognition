...
def trainMLP():
    ...
    if os.path.exists('models/hog_mlp.txt'):
        with open('models/hog_mlp.txt', 'rb') as file:
            mlp = pickle.load(file)
        file.close()
    else:
        mlp = MLPClassifier(max_iter=500)
        mlp.fit(hog, features_Y)
        with open('models/hog_mlp.txt', 'wb') as file:
            pickle.dump(mlp, file)
        file.close()
    test(mlp,"Multilayer Perceptron","HOG",X_test1,y_test1,5)

    if os.path.exists('models/lbp_mlp.txt'):
        with open('models/lbp_mlp.txt', 'rb') as file:
            mlp = pickle.load(file)
        file.close()
    else:
        mlp = MLPClassifier(max_iter=500)
        mlp.fit(lbp, features_Y)
        with open('models/lbp_mlp.txt', 'wb') as file:
            pickle.dump(mlp, file)
        file.close()
    test(mlp,"Multilayer Perceptron","LBP",X_test2,y_test2,28)

    if os.path.exists('models/harlick_mlp.txt'):
        with open('models/harlick_mlp.txt', 'rb') as file:
            mlp = pickle.load(file)
        file.close()
    else:
        mlp = MLPClassifier(max_iter=500)
        mlp.fit(harlick, features_Y)
        with open('models/harlick_mlp.txt', 'wb') as file:
            pickle.dump(mlp, file)
        file.close()
    test(mlp,"Multilayer Perceptron","Haralick",X_test3,y_test3,36) 
os.chdir("../Comparison/")
trainMLP()
print("------------------------------------------------------------------------------------------------------------------------------")